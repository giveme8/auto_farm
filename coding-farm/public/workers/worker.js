...
function getFeatureCheckMessage(key, fallback) {
  return featureCheckMessages?.[key] || fallback;
}
...
async function checkUserCodeFeatures(stripped) {
  const codeFeatureChecks = [
    ...
    // ———————————— functions（自定义函数） ————————————
    {
      feature: "functions",
      pattern: /\bfunction\b/,
      messageKey: "functionsLocked",
      message: "自定义函数功能尚未解锁",
    },
    {
      feature: "functions",
      pattern: /=\s*\([^)]*\)\s*=>/,
      messageKey: "arrowFunctionsLocked",
      message: "箭头函数功能尚未解锁",
    },
    ...
    // ———————————— loops（循环） ————————————
    {
      feature: "loops",
      pattern: /(^|[^\w])while\s*\(/,
      messageKey: "whileLoopsLocked",
      message: "while 循环功能尚未解锁",
    },
    {
      feature: "loops",
      pattern: /(^|[^\w])for\s*\(/,
      messageKey: "forLoopsLocked",
  ...
  for (const rule of codeFeatureChecks) {
    if (rule.pattern.test(stripped)) {
      ...
      if (!unlocked) {
        const message = getFeatureCheckMessage(
          rule.messageKey,
          rule.message
        );
...
onmessage = (e) => {
  ...
  if (data.type === "run") {
    ...
    featureCheckMessages = data.featureChecks || null;
    return;
  }
};
